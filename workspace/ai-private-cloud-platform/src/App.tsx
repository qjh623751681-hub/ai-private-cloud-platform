...
function App() {
  ...
  const renderFileManagement = () => {
    ...
    const getStatusColor = (status) => {
      const colors = { '已完成': '#10B981', '处理中': '#1677FF', '失败': '#F5222D' };
    ...
    const columns = [
      ...
      { title: '大小', dataIndex: 'size', key: 'size', width: 80, render: (size) => `${size} MB` },
      { title: '状态', dataIndex: 'status', key: 'status', width: 90, render: (status) => <Tag color={getStatusColor(status)}>{status}</Tag> },
  ...
  const renderDatasetManagement = () => {
    const getStatusColor = (status) => {
      const colors = { '草稿': '#6B7280', '进行中': '#1677FF', '已完成': '#10B981', '已发布': '#52c41a', '已锁定': '#8B5CF6' };
      return colors[status] || '#6B7280';
    };
    ...
    const columns = [
      ...
      { title: '类型', dataIndex: 'type', key: 'type', width: 80 },
      { title: '样本数', dataIndex: 'samples', key: 'samples', width: 100, render: (v) => v.toLocaleString() },
      { title: '大小', dataIndex: 'size', key: 'size', width: 80, render: (size) => `${size} GB` },
      { title: '状态', dataIndex: 'status', key: 'status', width: 90, render: (status) => <Tag color={getStatusColor(status)}>{status}</Tag> },
      { title: '进度', dataIndex: 'progress', key: 'progress', width: 120, render: (p) => <Progress percent={p} size="small" /> },
